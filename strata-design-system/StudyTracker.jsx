...
const TOPICS = [
  ...
  },
  {
    ...
    subtopics: [
      ...
      { id: "ra-2", title: "State colocation vs. global state trade-offs", xp: 30 },
  ...
  },
  {
    ...
    subtopics: [
      ...
      { id: "sd-2", title: "Cache invalidation & stale-while-revalidate", xp: 30 },
  ...
  },
  {
    ...
    subtopics: [
      ...
      { id: "pf-3", title: "Rendering strategies: CSR / SSR / SSG / ISR", xp: 35 },
    ],
  },
  {
    id: "testing",
...
const ALL_SUBTOPICS = TOPICS.flatMap((t) => t.subtopics.map((s) => ({ ...s, topicId: t.id })));
...
const ACHIEVEMENTS = [
  ...
  },
  {
    ...
    check: (s) => TOPICS.some((t) => t.subtopics.every((sub) => s[sub.id] === "complete")),
  },
  {
    id: "halfway",
    title: "Halfway Point",
    description: "Reach 50% overall progress.",
    check: (s) => ALL_SUBTOPICS.filter((sub) => s[sub.id] === "complete").length / ALL_SUBTOPICS.length >= 0.5,
  },
  {
    id: "streak-3",
    title: "On a Roll",
  ...
  },
  {
    ...
    check: (s) => TOPICS.find((t) => t.id === "leadership").subtopics.every((sub) => s[sub.id] === "complete"),
  },
  {
    id: "all-cleared",
...
function StatusGlyph({ status }) {
  if (status === "complete") return <CheckCircle2 size={18} color={COLOR.emerald} strokeWidth={2} />;
  if (status === "in-progress") return <Circle size={18} color={COLOR.amber} fill={COLOR.amber} fillOpacity={0.25} strokeWidth={2} />;
...
function StatBlock({ label, value, color, mono = true }) {
  return (
    <div style={{ flex: "1 1 0", minWidth: 110 }}>
      <div className="strata-mono" style={{ fontSize: 11, color: COLOR.textMuted, textTransform: "uppercase" }}>
        {label}
      </div>
      <div className={mono ? "strata-mono" : ""} style={{ fontSize: 24, fontWeight: 600, color: color || COLOR.textPrimary, marginTop: 4 }}>
        {value}
      </div>
    </div>
  );
...
export default function StudyTracker() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await window.storage.set(STORAGE_KEY, JSON.stringify({ statusMap, streak, lastStudied }));
  ...
  const completedIds = useMemo(
    () => (statusMap ? ALL_SUBTOPICS.filter((s) => statusMap[s.id] === "complete") : []),
    [statusMap]
  );
  const totalXp = useMemo(() => completedIds.reduce((sum, s) => sum + s.xp, 0), [completedIds]);
  const progressPct = ALL_SUBTOPICS.length ? Math.round((completedIds.length / ALL_SUBTOPICS.length) * 100) : 0;
  const topicsCleared = TOPICS.filter((t) => t.subtopics.every((s) => statusMap?.[s.id] === "complete")).length;
  ...
  const unlockedAchievements = useMemo(() => {
    ...
    const ids = ACHIEVEMENTS.filter((a) => a.check(statusMap, streak)).map((a) => a.id);
    return new Set(ids);
  }, [statusMap, streak]);
  ...
  const filteredTopics = useMemo(() => {
    ...
    return TOPICS.map((t) => {
      ...
      const subMatches = t.subtopics.filter((s) => s.title.toLowerCase().includes(q));
  ...
  const cycleStatus = useCallback((subId) => {
    ...
    setLastStudied((prevDate) => {
      ...
      setStreak((prevStreak) => (prevDate && daysBetween(prevDate, today) === 1 ? prevStreak + 1 : 1));
      return today;
    });
  }, []);

  const resetProgress = useCallback(async () => {
    if (!window.confirm("Reset all progress? This clears every checked objective, your streak, and XP.")) return;
    setStatusMap({});
    setStreak(0);
    setLastStudied(null);
    try {
  ...
  if (statusMap === null) {
    return (
      <div className="strata-root" style={{ padding: 40, fontSize: 13, color: COLOR.textMuted }}>
        <style>{STYLES}</style>
  ...
  return (
    <div className="strata-root" style={{ minHeight: "100%", padding: "24px 16px 40px" }}>
      <style>{STYLES}</style>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          <div>
            <div className="strata-mono" style={{ fontSize: 11, color: COLOR.amber, textTransform: "uppercase", marginBottom: 4 }}>
              Field Research Console
            </div>
            <div className="strata-display" style={{ fontSize: 28, fontWeight: 600 }}>
              STRATA
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
            ...
            <div className="strata-mono" style={{ fontSize: 16, fontWeight: 600, color: COLOR.amber }}>
              {rank.name}
              {nextRank && (
        ...
        {/* Telemetry */}
        <div className="strata-card" style={{ display: "flex", flexWrap: "wrap", gap: 16, padding: 16, marginBottom: 16 }}>
          <StatBlock label="Progress" value={`${progressPct}%`} color={COLOR.amber} />
          <StatBlock label="Topics cleared" value={`${topicsCleared}/${TOPICS.length}`} color={COLOR.teal} />
          <StatBlock
            label="Streak"
            value={
              <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
                <Flame size={18} color={COLOR.rose} fill={streak > 0 ? COLOR.rose : "transparent"} />
                {streak}d
              </span>
            }
            color={COLOR.rose}
        ...
        <div style={{ position: "relative", marginBottom: 16 }}>
          <Search size={16} color={COLOR.textMuted} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)" }} />
          <input
            className="strata-input"
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search topics or objectives…"
            style={{ width: "100%", padding: "10px 12px 10px 36px", borderRadius: 10, fontSize: 14, fontFamily: FONT.body }}
          />
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
          ...
          {filteredTopics.map((topic) => {
            ...
            const doneCount = subs.filter((s) => statusMap[s.id] === "complete").length;
            const topicStatus = doneCount === subs.length ? "complete" : doneCount > 0 ? "in-progress" : "not-started";
            const isOpen = expanded.has(topic.id);
            return (
              <div key={topic.id} className="strata-card" style={{ overflow: "hidden" }}>
                <button
                ...
                >
                  ...
                  <span style={{ flex: 1, fontSize: 14, fontWeight: 500 }}>{topic.title}</span>
                  <span className="strata-mono" style={{ fontSize: 12, color: COLOR.textMuted }}>
                    {doneCount}/{subs.length}
                  </span>
                  {isOpen ? <ChevronDown size={16} color={COLOR.textMuted} /> : <ChevronRight size={16} color={COLOR.textMuted} />}
                </button>
                    ...
                    {subs.map((sub, i) => {
                      ...
                      return (
                        <button
                          ...
                          style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 14px 10px 30px" }}
                        >
                          <span className="strata-mono" style={{ fontSize: 11, color: COLOR.textMuted, width: 18 }}>
                            {String(i + 1).padStart(2, "0")}
                          </span>
                          <span className={justCompleted === sub.id ? "strata-stamp" : ""} style={{ display: "flex" }}>
                          ...
                          <span
                            style={{
                              ...
                              fontSize: 13.5,
                              textAlign: "left",
                              color: status === "complete" ? COLOR.textMuted : COLOR.textPrimary,
                              textDecoration: status === "complete" ? "line-through" : "none",
                            }}
                          >
                            {sub.title}
                          </span>
                          <span className="strata-mono" style={{ fontSize: 11, color: status === "complete" ? COLOR.emerald : COLOR.textMuted }}>
                            +{sub.xp} XP
                          </span>
                        </button>
                      );
        ...
        <div style={{ marginBottom: 8 }}>
          <div className="strata-mono" style={{ fontSize: 11, color: COLOR.textMuted, textTransform: "uppercase", marginBottom: 10 }}>
            Field commendations
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 10 }}>
            {ACHIEVEMENTS.map((a) => {
              ...
              return (
                <div key={a.id} className={`strata-patch${unlocked ? " unlocked" : ""}`} style={{ padding: 12, display: "flex", gap: 10 }}>
                  {unlocked ? <Trophy size={18} color={COLOR.amber} /> : <Lock size={18} color={COLOR.textMuted} />}
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: unlocked ? COLOR.amber : COLOR.textMuted }}>{a.title}</div>
                    <div style={{ fontSize: 11.5, color: COLOR.textMuted, marginTop: 2 }}>{a.description}</div>
                  </div>
                </div>
              );
            })}
        ...
        {/* Footer / reset */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 24, paddingTop: 16, borderTop: `1px solid ${COLOR.line}` }}>
          <span className="strata-mono" style={{ fontSize: 11, color: COLOR.textMuted }}>
            {saveState === "saving" ? "Syncing…" : saveState === "error" ? "Sync failed — progress kept locally this session" : "Synced"}
          </span>
          <button
            onClick={resetProgress}
            className="strata-mono"
            style={{ display: "flex", alignItems: "center", gap: 6, background: "transparent", border: `1px solid ${COLOR.line}`, color: COLOR.textMuted, borderRadius: 8, padding: "6px 10px", fontSize: 11, cursor: "pointer" }}
          >
