...
- **IBM Plex Mono** is the instrument face: anything that is a *number, code, or status* is mono. This is a hard rule, not a style preference — it's how the eye tells "data" apart from "prose" at a glance.
...
- **Objective row** (subtopic) — two-digit sequence number, status icon, title, XP value. Sequence numbers are used here deliberately: within a topic, objectives *do* have a real learning order. Topics themselves are not numbered, because studying them in a fixed global order is not actually true.
